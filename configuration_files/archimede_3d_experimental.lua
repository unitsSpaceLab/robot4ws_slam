...
options = {
  ...
  tracking_frame = "imu_link", -- "camera_accel_frame", -- The ROS frame ID of the frame that is tracked by the SLAM algorithm
...
TRAJECTORY_BUILDER_3D.real_time_correlative_scan_matcher.linear_search_window = 0.15 --0.15
TRAJECTORY_BUILDER_3D.real_time_correlative_scan_matcher.angular_search_window = math.rad(1) --math.rad(1)
...
TRAJECTORY_BUILDER_3D.min_range = 0.5 --1
TRAJECTORY_BUILDER_3D.max_range = 4.5
...
TRAJECTORY_BUILDER_3D.ceres_scan_matcher.only_optimize_yaw = false --false
TRAJECTORY_BUILDER_3D.ceres_scan_matcher.translation_weight = 5e2 --5
TRAJECTORY_BUILDER_3D.ceres_scan_matcher.rotation_weight = 1e1 --4e2
TRAJECTORY_BUILDER_3D.submaps.num_range_data = 320 --160
TRAJECTORY_BUILDER_3D.imu_gravity_time_constant = 10 --10 -- check Exp Value 

--GLOBAL SLAM

POSE_GRAPH.optimize_every_n_nodes = 180 --90 --consider to set it to zero, it seems it gives a lot of problem at each optimization
POSE_GRAPH.matcher_translation_weight = 5e3 --5e2
POSE_GRAPH.matcher_rotation_weight = 1.6e1 --1.6e3
...
POSE_GRAPH.constraint_builder.fast_correlative_scan_matcher_3d.linear_z_search_window = 0.1 --1
